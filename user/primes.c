...
sieve (int left_pipe[2])
{
  ...
  if (pid == 0)
    {
      // Process con: đóng đầu vào của pipe bên trái và đầu ra của pipe bên
      // phải
      close (left_pipe[0]);
      close (right_pipe[1]);
  ...
  else
    {
      ...
      // Đọc các số từ pipe bên trái và gửi chúng qua pipe bên phải nếu không
      // phải là bội của prime
